...
struct my_proc_stat
{
	...
	// pid_t pid; //pid_t int
	struct proc_page_stat proc_page_count;
};

#define PROC_PLOGFILE "/home/gesefudiao/pageslogfile.txt"
#define PROC_ALOGFILE "/home/gesefudiao/addslogfile.txt"
#define PROC_PIDFILE "/home/gesefudiao/in"

unsigned long long f_offset = 0;
unsigned long long a_offset = 0;
struct file *outplogfile = NULL;
...
static struct file *file_open(const char *path, int flags, int rights)
{
	...
	oldfs = get_fs();
	set_fs(get_ds());
	//	printk("gesefudiao:in file open\n");
	filp = filp_open(path, flags, rights);
	//	printk("gesefudiao:299:%p\n", filp);
	set_fs(oldfs);
	if (IS_ERR(filp))
	{
		err = PTR_ERR(filp);
...
static int file_write(struct file *file, unsigned long long offset, unsigned char *data, unsigned int size)
{
	...
	oldfs = get_fs();
	set_fs(get_ds());
	//	printk("gesefudiao:in file_write");
...
static void count_pages_tsk(struct task_struct *task, struct proc_page_stat *page_stat, unsigned char *buf)
{
	...
	page_stat->scanned_pages = 0;
	page_stat->active_file_pages = 0;
	page_stat->active_ano_pages = 0;
	page_stat->inactive_file_pages = 0;
	page_stat->inactive_ano_pages = 0;
	//	printk("gesefudiao-438:count task pages %d,%s in count_pages_tsk()\n", task->pid, task->comm);
	//	printk("gesefudiao-439:%d %20ld %20ld %20ld %20ld\n",task->pid,page_stat->active_file_pages,page_stat->active_ano_pages,page_stat->inactive_file_pages,page_stat->inactive_ano_pages);
	for (vma = task->mm->mmap; vma != NULL; vma = vma->vm_next)
	{
		...
		{
			p = NULL;
			p = follow_page(vma, addr, FOLL_GET);
			if (IS_ERR_OR_NULL(p))
				continue;
			else
			{
 ...
 * @param  srcStr     源字符串
 * @param  desStr     目标字符串
 * @return         1表示字符串相同，0表示字符串不相同
 */
static int eqstr(char *srcStr, char *desStr)
{
	...
	//	printk("gesefudiao:%s,%s", srcStr, desStr);
	while (*srcStr && *desStr)
	{
		if (*srcStr++ != *desStr++)
		{
...
static int fillpid(int *count)
{
	...
	if (infile == NULL)
	{
		...
		infile = file_open(PROC_PIDFILE, O_RDWR, 0644);
		file_read(infile, in_offset, (char *)inbuf, 512);
		while (inbuf[j] && (isalnum(inbuf[j]) || isspace(inbuf[j])))
		{
			...
			else
			{
				temp[i] = '\0';
				j++;
				kstrtouint(temp, 10, &proc_pid[pid_count]);
				printk("gesefudiao:555 %d\n", proc_pid[pid_count]);
				memset(temp, 0, 20);
				i = 0;
				pid_count++;
			}
			printk("gesefudiao:560 %d\n", pid_count);
		}
		temp[i++] = '\0';
	...
	*count = pid_count + 1;
	printk("gesefudiao:567 %d\n", *count);
	file_close(infile);
	infile = NULL;
	return 0;
}

static int count_from_str(void)
{
	struct task_struct *task = NULL;
	int proc_count = 0;
	int i;

	for_each_process(task) //记录VM进程
	{
		//	printk("gesefudiao:qemu task %d,%s\n", task->pid, task->comm);
		if (eqstr(task->comm, "qemu-system-x86")) //该进程为虚拟机进程
		{
			procs[proc_count].proc_task = task;
			proc_count++;
			printk("gesefudiao:590 qemu task %d,%s\n", task->pid, task->comm);
		}
	}
	...
	{
		if (procs[i].proc_task->mm == NULL)
			continue;
		task = procs[i].proc_task;
		//				printk("gesefudiao:scan task %d,%s\n", task->pid, task->comm);
		//				scan_add_tsk(task, outplogfile, f_offset, pbuf);
		if (proc_pid[1] == 1)
...
static void count_from_pid(int *count)
{
	struct task_struct *task = NULL;
	struct pid *p_pid;
	int pid_n = -1;
	...
	for (i = 3; i < *count; i++)
	{
		...
		if (pid_n != 0 && pid_n != -1)
		{
			...
			procs[j++].proc_task = pid_task(p_pid, PIDTYPE_PID);
		}
	}
	for (i = 0; i < j; i++)
	{
...
static struct mm_slot *get_mm_slot(struct mm_struct *mm)
{
	...
	hash_for_each_possible(mm_slots_hash, slot, link, (unsigned long)mm) if (slot->mm == mm) return slot;
...
static int ksm_scan_thread(void *nothing)
{
	...
	while (!kthread_should_stop())
	{
		...
		{
			//			printk("gesefudiao:%d\n", scan_flags);
			// printk("gesefudiao:ksm scan thread start\n");
			// printk("gesefudiao:1927 enter the loop\n");
			// add by gesefudiao
